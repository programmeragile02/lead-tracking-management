  ...
  export async function PATCH(
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (isNaN(newNext.getTime())) {
          throw new Error("Invalid nextActionAt date");
        }

        if (
          existing.nextActionAt &&
          new Date(existing.nextActionAt).toDateString() ===
            newNext.toDateString()
        ) {
          throw new Error("Tanggal tindak lanjut sama dengan sebelumnya");
        }
