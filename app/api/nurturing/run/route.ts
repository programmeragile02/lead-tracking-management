...
export async function POST(req: NextRequest) {
  try {
    ...
    // -------------------------------------------------------------
    // C0. Kirim STEP PERTAMA (order = 1) untuk lead ACTIVE yang
    //     belum pernah dikirimi pesan nurturing sama sekali
    // -------------------------------------------------------------
    const firstStepType = typeByOrder.get(1);

    if (firstStepType) {
      const firstStepLeads = await prisma.lead.findMany({
        where: {
          nurturingStatus: NurturingStatus.ACTIVE,
          nurturingCurrentStep: null,
          nurturingLastSentAt: null,
          // skip yg sudah closing / hot
          NOT: {
            status: {
      ...
      for (const lead of firstStepLeads) {
        processed++;

        if (!lead.phone) {
          errors.push({
            leadId: lead.id,
            reason: "Lead tidak punya nomor telepon (step pertama)",
          });
          continue;
        }
        ...
        if (!lead.salesId) {
          errors.push({
            leadId: lead.id,
            reason: "Lead tidak punya salesId (step pertama)",
          });
          continue;
        }
        ...
        const messageContent = renderTemplate(firstStepType.waTemplateBody, {
          nama_lead: lead.name,
        ...
        if (!messageContent) {
          errors.push({
            leadId: lead.id,
            reason: "Template kosong untuk step nurturing order=1",
          });
          continue;
        }
        ...
        try {
          ...
          const result = await sendWaMessage({
            ...
            meta: {
              leadId: lead.id,
              followUpTypeCode: firstStepType.code,
              kind: "NURTURING_STEP",
              stepOrder: 1,
            },
          });
          ...
          console.error(
            `[NURTURING] gagal kirim WA step 1 untuk lead ${lead.id}:`,
            e
          );
          errors.push({
            leadId: lead.id,
            reason: "Gagal kirim WA (step 1)",
          });
          continue;
        }
        ...
        // Catat ke LeadFollowUp (auto nurturing)
        await prisma.leadFollowUp.create({
          data: {
            leadId: lead.id,
            salesId: lead.salesId,
            typeId: firstStepType.id,
            note: "Pesan Otomatis - Step 1",
        ...
        const hasNext = typeByOrder.has(2);

        await prisma.lead.update({
          where: { id: lead.id },
          data: {
            nurturingCurrentStep: 1,
            nurturingLastSentAt: nowStep,
            nurturingStartedAt: lead.nurturingStartedAt ?? nowStep,
    ...
    // -------------------------------------------------------------
    // C. Ambil lead yang nurturing-nya ACTIVE & sudah pernah dikirimi step
